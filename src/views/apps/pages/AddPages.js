...
const schema = yup.object().shape({
    pageName: yup.string().trim().required(),
    pagePath: yup.string().trim().required(),
...
const AddPages = ({ show, setShow, defaultData }) => {
    ...
    const onSubmit = async (data) => {
        ...
        if (defaultData) {
            requestData = {
                ...data, id: defaultData?.PageID || ''
            }
        }

        const [error, result] = await asyncWrap(
            axios.post("/page", requestData)
        );
        if (!result) {
            console.log(error);
    ...
    useEffect(() => {
        if (defaultData && show) {
            const mappedData = {
                ...
                pagePath: defaultData.PagePath || '',
                moduleId: defaultData?.ModuleID || ''
            };
                ...
                >
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <FormControl fullWidth sx={{ mb: 6 }}>
                            ...
                            />
                            {errors.pageName && <FormHelperText sx={{ color: 'error.main' }}>{errors.pageName.message}</FormHelperText>}
                        </FormControl>
                        {/* </Grid> */}
                        {/* <Grid item xs={12}> */}
                        <FormControl fullWidth sx={{ mb: 6 }}>
                        ...
                        <FormControl fullWidth>
                            <InputLabel id='country-select'>Select Module</InputLabel>
                            <Select
                                fullWidth
                            ...
                            >
                                ...
                                {moduleData?.map((item, index) => (
                                    <MenuItem key={index} value={item?.ModuleID}>{item?.ModuleName}</MenuItem>
                                ))}
                            </Select>
                            {errors.moduleId && (
