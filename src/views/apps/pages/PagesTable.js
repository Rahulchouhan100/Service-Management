...
// ** Custom Components Imports
import TableHeader from 'src/views/apps/pages/TableHeader'
...
const UserList = ({ show, setShow, setDefaultData, defaultData, setShowPagesDetails, showPagesDetails, deletePages, setDeletePages }) => {
  const { data: rows, total, loading, currentPage, gotoPage, setPerPage, perPage, fetchData } = usePagination('/page');
  ...
  return (
    <Grid container spacing={6}>
      <Grid item xs={12}>
        <Card>
          <TableHeader urlParamsData={urlParamsData} setUrlParamsData={setUrlParamsData} plan={plan} value={value} handleFilter={handleFilter} handlePlanChange={handlePlanChange} show={show} setShow={setShow} />
          <DataGrid
