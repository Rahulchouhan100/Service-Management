...
const TreeViewCustomized = ({ setParentCategoryId, setParentCategoryData, parentCategoryId, parentCategoryData, direction, setShow, setShowProperty, show }) => {
  const ExpandIcon = <Icon icon={direction === 'rtl' ? 'mdi:chevron-left' : 'mdi:chevron-right'} />
  const [parentCategory, setParentCategory] = useState([])

  const getParentCategoryData = async () => {
    const [error, result] = await asyncWrap(axios.get("/category/parent"));
